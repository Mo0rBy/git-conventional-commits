...
module.exports = function (convention, commitAnchor = 'HEAD') {
    ...
    function parseCommitSubject(commit) {
        ...
        if (!msgMatch) { // Must match with configured regex pattern
            throw new Error(`
                Invalid commit subject: Does not match required regex.
                Required regex: ${convention.msgRegex}
            `);
        ...
        // Must contain a 'type' and 'description'
        if (!conventionalSubject.type) {
            throw new Error(`
                Invalid commit subject: Capturing group 'type' is empty.
                You must include a 'type' for Conventional Commits.
                Required regex: ${convention.msgRegex}
            `);
        }
        if (!conventionalSubject.description) {
            throw new Error(`
                Invalid commit subject: Capturing group 'description' is empty.
                You must include a 'description' for Conventional Commits.
                Required regex: ${convention.msgRegex}
            `);
        }

        // TODO: Need to add msgMergeMatch and msgRevertMatch logic here

        // Warn for unexpected type and scope
        // if configuration values are provided
        if (conventionalSubject.type &&
